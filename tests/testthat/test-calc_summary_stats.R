test_that("use, single trace", {
  ...
  expect_equal(sum_stats$mean, -70.5839, tolerance = 0.00001)
  expect_true("stderr_mean" %in% names(sum_stats))
  expect_equal(sum_stats$stderr_mean, 0.5045, tolerance = 0.0001)

  expect_true("stdev" %in% names(sum_stats))
  expect_equal(sum_stats$stdev, 1.6816, tolerance = 0.0001)

  expect_true("variance" %in% names(sum_stats))
  expect_equal(sum_stats$variance, 2.8279, tolerance = 0.0001)
  ...
  expect_equal(sum_stats$hpd_interval_low, -74.1527, tolerance = 0.0001)

  expect_true("hpd_interval_high" %in% names(sum_stats))
  expect_equal(sum_stats$hpd_interval_high, -68.6852, tolerance = 0.0001)

  expect_true("act" %in% names(sum_stats))
  expect_equal(sum_stats$act, 1000, tolerance = 0.0001)

  expect_true("ess" %in% names(sum_stats))
  expect_equal(sum_stats$ess, 10, tolerance = 0.0001)
